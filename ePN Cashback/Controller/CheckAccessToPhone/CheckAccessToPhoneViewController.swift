//
//  CheckAccessToPhoneViewController.swift
//  Backit
//
...
class CheckAccessToPhoneViewController: UIViewController {
    
    // MARK: - Instance Properties
    
    var viewModel: CheckAccessToPhoneViewModel!
    ...
    private let leftButton = EPNButton(style: .secondary, size: .large2)
    private let rightButton = EPNButton(style: .primary, size: .large2)
    ...
    private func setupView() {
        ...
        leftButton.text = viewModel.alertLeftButtonTitle
        leftButton.handler = { [weak self] button in
            self?.onLeftButtonTouchUpInside(sender: button)
        }
        
        rightButton.text = viewModel.alertRightButtonTitle
        rightButton.handler = { [weak self] button in
            self?.onRightButtonTouchUpInside(sender: button)
        }
    ...
    @objc private func onLeftButtonTouchUpInside(sender: Any) {
        hideFloatingView(completion: { [unowned self] in
            self.viewModel.showNoAccessEditPhone()
        })
    }
    
    @objc private func onRightButtonTouchUpInside(sender: Any) {
        hideFloatingView(completion: { [unowned self] in
            self.viewModel.showYesAccessEditPhone()
        })
    }
}
